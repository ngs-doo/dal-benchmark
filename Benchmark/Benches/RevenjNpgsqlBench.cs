...
namespace Benchmark
{
	class RevenjNpgsqlBench
	{
		...
		static NpgsqlConnection Setup()
		{
			...
			var com = new NpgsqlCommand(DbInitScript) { Connection = conn };
			conn.Open();
			com.ExecuteNonQuery();
			return conn;
		}
		...
		class StandardBench : IBench<StandardRelations.Invoice>
		{
				...
				using (var readerHead = comHead.ExecuteReader())
				{
					if (readerHead.Read())
					{
						invoice = new StandardRelations.Invoice
						{
							...
							reference = readerHead.GetString(7),
			...
			public void Update(IEnumerable<StandardRelations.Invoice> values)
			{
				...
				using (var comChildDelete = Conn.CreateCommand())
				{
					...
					comHead.Parameters.AddRange(new[] { ph1, ph2, ph3, ph4, ph5, ph6, ph7, ph8, ph9, puri });
					comChildInsert.Parameters.AddRange(new[] { pc1, pc2, pc3, pc4, pc5, pc6, pc7 });
					comChildUpdate.Parameters.AddRange(new[] { pc1, pc2, pc3, pc4, pc5, pc6, pc7 });
					comChildDelete.Parameters.AddRange(new[] { pc1, pc2 });
					foreach (var item in values)
					{
						puri.Value = item.URI;
		...
		class ComplexBench : IBench<ComplexRelations.BankScrape>
		{
				...
				using (var readerHead = comHead.ExecuteReader())
				{
					if (readerHead.Read())
					{
						scrape = new ComplexRelations.BankScrape
						{
							...
							externalId = readerHead.GetString(4),
			...
			public void Insert(IEnumerable<ComplexRelations.BankScrape> values)
			{
				...
				using (var comDetail = Conn.CreateCommand())
				{
					...
					var ph4 = new NpgsqlParameter("info", DbType.Object);
			...
			public void Update(IEnumerable<ComplexRelations.BankScrape> values)
			{
				...
				using (var comDetailDelete = Conn.CreateCommand())
				{
					...
					var pd7 = new NpgsqlParameter("amount", DbType.Decimal);
					comInfo.Parameters.Add(puri);
					comHead.Parameters.AddRange(new[] { puri, ph1, ph2, ph3, ph4, ph5, ph6, ph7 });
					comChildInsert.Parameters.AddRange(new[] { pc1, pc2, pc3, pc4, pc5, pc6 });
					comChildUpdate.Parameters.AddRange(new[] { pc1, pc2, pc3, pc4, pc5, pc6 });
					comChildDelete.Parameters.AddRange(new[] { pc1, pc2 });
					comDetailInsert.Parameters.AddRange(new[] { pd1, pd2, pd3, pd4, pd5, pd6, pd7 });
					comDetailUpdate.Parameters.AddRange(new[] { pd1, pd2, pd3, pd4, pd5, pd6, pd7 });
					comDetailDelete.Parameters.AddRange(new[] { pd1, pd2, pd3 });
					foreach (var item in values)
					{
						puri.Value = item.URI;
